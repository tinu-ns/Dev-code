...
const areFiltersApplied = filterOptions.taxable || filterOptions.condition || filterOptions.base;
  ...
  const handleEdit = (id) => {
    ...
    const updatedAllowance = { ...allowanceToEdit, name: "Updated Name" }; // Example update
  console.log("Edit clicked for ID: ", id);

    axios
      .put(`http://localhost:5000/api/allowances/${id}`, updatedAllowance)
      .then((response) => {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`http://localhost:5000/api/allowances/${id}`)
      .then(() => {
        setAllowancesData((prev) => prev.filter((allowance) => allowance.id !== id));
        alert("Allowance deleted successfully!");
      })
...
const removeFilter = (filter) => {
  ...
  return (
    ...
    <button  onClick={ () => {
      disabled={false}
       console.log("Edit clicked for ID:", allowance.id);
       handleEdit(allowance.id)
    }}>
      <FaEdit />
    </button>
    <button onClick={() => {
       console.log("Delete clicked for ID:", allowance.id);
       handleDelete(allowance.id);
    }}>
      <FaTrash />
    </button>
  </div>

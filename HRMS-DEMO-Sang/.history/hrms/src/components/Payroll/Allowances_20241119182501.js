...
const handleCreateAllowance = async (newAllowance) => {
  try {
    ...
    console.error("Error creating allowance:", error.response?.data || error.message);
...
const areFiltersApplied = filterOptions.taxable || filterOptions.condition || filterOptions.base;
  ...
  */}
  ...
  //       prev.map((allowance) =>
  //         allowance.id === id ? { ...allowance, ...response.data } : allowance
  //       )
  //     );
  ...
  //   setFilteredData((prev) =>
  //     prev.map((allowance) =>
  //       allowance.id === id ? { ...allowance, isEditing: true } : allowance
  //     )
  //   );
  // };
  
  // // const handleSaveEdit = async (id, updatedAllowance) => {
  // //   try {
  // //     const response = await axios.patch(`http://localhost:5000/api/allowances/${id}`, updatedAllowance);
  // //     setAllowancesData((prev) =>
  // //       prev.map((allowance) =>
  ...
  // //     setFilteredData((prev) =>
  // //       prev.map((allowance) =>
  // //         allowance.id === id ? { ...response.data, isEditing: false } : allowance
  // //       )
  // //     );
  // //     alert("Allowance updated successfully!");
  // //   } catch (error) {
  ...
  // const handleSaveEdit = async (id) => {
  //   try {
  //     const updatedAllowance = filteredData.find((allowance) => allowance.id === id);
  //     const response = await axios.put(`http://localhost:5000/api/allowances/${id}`, updatedAllowance);
  ...
  const handleEdit = (id) => {
    setEditingId(id);
    const allowanceToEdit = allowancesData.find((allowance) => allowance.id === id);
    if (allowanceToEdit) {
      setEditData(allowanceToEdit);
    }
  };
  ...
  const cancelEdit = () => {
    setEditingId(null); // Simply reset editing mode
    setEditData({});
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error("Error deleting allowance:", error.response?.data || error.message);
      alert("Failed to delete allowance.");
    }
  };
    ...
    <div className="allowances-container">
      <header className="allowances-header">
        ...
        <div className="controls">
          ...
          <button className="create-btn" onClick={() => navigate("/allowances/create")}> 
            <FaPlus /> Create
          </button>
        </div>
      </header>
        ...
        <div className="card-view">
          {filteredData.map((allowance) => (
            <div className="allowance-card" key={allowance.id}>
              <div className="card-icon">{allowance.name.split(" ")[0][0]} </div>
              ...
              {/* {editingId === allowance.id ? ( */}
                ...
                <div className="card-content">
                  <input
                    ...
                    onChange={(e) => handleEditChange(allowance.id, "name", e.target.value)}
                  />
                  <input
                    type="number"
                    name="amount"
                    value={allowance.amount}
                    onChange={(e) => handleEditChange(allowance.id, "amount", e.target.value)}
                  />
                  <button onClick={() => handleSaveEdit(allowance.id)}>
                    <FaCheck />
                  </button>
                  <button onClick={() => cancelEdit(allowance.id)}>
                    <FaTimes />
                  </button>
                </div>
              ) : (

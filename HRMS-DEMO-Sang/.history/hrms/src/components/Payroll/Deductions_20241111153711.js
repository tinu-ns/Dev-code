      ...
      {isFilterVisible && (
        <div className="filter-popup" ref={filterRef}>
          <div className="filter-form">
            <div className="group" style={{display: "flex", gap: "20", justifyContent: "space-between", marginBottom: "20px"}}>
            ...
            <div className="filter-row">
              ...
              >
                <option value="">Select Condition Based</option>
      ...
      {view === "card" ? (
        <div className="card-view">
          {filteredData.map((deduction) => (
            <div className="allowance-card" key={deduction.id}>
              <div className="card-icon">{deduction.code}</div>
              <div className="card-content">
                <h3>{deduction.name}</h3>
                <p>Employee Rate: {deduction.employeeRate}</p>
                <p>Employer Rate: {deduction.employerRate}</p>
                <p>One Time Deduction: {deduction.oneTimeDeduction}</p>
                <p>Taxable: {deduction.taxable}</p>
              </div>
            </div>
          ))}
        </div>

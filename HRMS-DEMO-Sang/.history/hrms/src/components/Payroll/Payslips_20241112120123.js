...
const Payslips = ({data} = []) => {
  ...
  const applyFilters = () => {
      ...
      if (filterCriteria.grossPayLessThan) { // Corrected field name
        updatedData = updatedData.filter(item => parseFloat(item.grossPay.replace('ETB ', '')) <= parseFloat(filterCriteria.grossPayLessThan));
    }
    if (filterCriteria.grossPayGreaterOrEqual) { // Corrected field name
    ...
    if (filterCriteria.deductionLessThan) { // Corrected field name
        updatedData = updatedData.filter(item => parseFloat(item.deduction.replace('ETB ', '')) <= parseFloat(filterCriteria.deductionLessThan));
    }
    if (filterCriteria.deductionGreaterOrEqual) { // Corrected field name
    ...
    if (filterCriteria.netPayLessThan) { // Corrected field name
        updatedData = updatedData.filter(item => parseFloat(item.netPay.replace('ETB ', '')) <= parseFloat(filterCriteria.netPayLessThan));
    }
    if (filterCriteria.netPayGreaterOrEqual) { // Corrected field name

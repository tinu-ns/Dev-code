...
const saveNewPayslip = () => {
  ...
  setPayslips(prevPayslips => {
      ...
      // Update the filteredPayslips as well to show the new entry
    ...
    <div className="payslip-list">
                {payslips.map(payslip => (
                    <div key={payslip.id} className="payslip-item">
                        <h4>{payslip.employee}</h4>
                        <p>Start Date: {payslip.startDate.toString()}</p>
                        <p>End Date: {payslip.endDate.toString()}</p>
                        {/* Include additional payslip details if desired */}
                    </div>
                ))}
            </div>

...
const Payslips = ({data} = []) => {
    ...
    const [showFilterModal, setShowFilterModal] = useState(data);
    ...
    onst [isFilterPopupOpen, setIsFilterPopupOpen] = useState(false);
  ...
  const applyFilters = () => {
      ...
      if (filterCriteria.grossPayLess) {
          updatedData = updatedData.filter(item => item.grossPay <= parseFloat(filterCriteria.grossPayLess));
      }
      if (filterCriteria.grossPayMore) {
          updatedData = updatedData.filter(item => item.grossPay >= parseFloat(filterCriteria.grossPayMore));
      }
      if (filterCriteria.deductionLess) {
          updatedData = updatedData.filter(item => item.deduction <= parseFloat(filterCriteria.deductionLess));
      }
      if (filterCriteria.deductionMore) {
          updatedData = updatedData.filter(item => item.deduction >= parseFloat(filterCriteria.deductionMore));
      }
      if (filterCriteria.netPayLess) {
          updatedData = updatedData.filter(item => item.netPay <= parseFloat(filterCriteria.netPayLess));
      }
      if (filterCriteria.netPayMore) {
          updatedData = updatedData.filter(item => item.netPay >= parseFloat(filterCriteria.netPayMore));
      }

      setShowFilterModal(updatedData);
      toggleFilterModal();
  };

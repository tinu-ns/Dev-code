...
const allowancesData = [
  ...
  { id: 3, code: 'DA', name: ' Dearness Allowance', amount: 1500.0, oneTime: 'No', taxable: 'Yes', fixed: true },
              ...
              {filteredData.map((allowance) => (
                <tr key={allowance.id}>
                  ...
                  <td  className="sticky-column">
                    <button className="edit-btn" onClick={() => handleEdit(allowance.id)}><FaEdit /></button>
                    <button className="delete-btn" onClick={() => handleDelete(allowance.id)}><FaTrash /></button>
                  </td>
                </tr>
              ))}
            </tbody>

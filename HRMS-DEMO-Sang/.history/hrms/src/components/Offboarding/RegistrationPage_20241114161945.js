...
const ResignationPage = () => {
  ...
  const handleDeleteClick = (id) => {
    setNewResignation((prev) => prev.filter((res) => res.id !== id));
  };
  ...
  const handleSave = () => {
    if (isEditing) {
      ...
      setResignations((prev) =>
        prev.map((res) =>
          res.id === currentId ? { ...res, ...resignationData } : res
        )
      );
    } else {
      // Add new resignation
      setResignations((prev) => [
        ...prev,
        { id: prev.length + 1, ...resignationData },
      ]);
    }
    setShowPopup(false);
    setResignationData({ name: '', title: '', status: 'Approved' });
  };

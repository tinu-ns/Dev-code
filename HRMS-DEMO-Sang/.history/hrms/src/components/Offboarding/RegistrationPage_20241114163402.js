...
const ResignationPage = () => {
 ...
 // const handleCreateClick = () => {
  ...
  {/*  const handleSave = () => {
    setData((prev) => [...prev, { id: prev.length + 1, ...newResignation }]);
...
{/*
  const handleSave = () => {
    if (isEditing) {
      // Edit existing resignation
      setData((prev) =>
        prev.map((res) =>
          res.id === currentId ? { ...res, ...data } : res
        )
      );
    } else {
      // Add new resignation
  ...
  const handleSave = () => {
    if (isEditing) {
      setData((prev) =>
        prev.map((res) =>
          res.id === currentId
            ? { ...res, name: newResignation.name, position: newResignation.title, status: newResignation.status }
            : res
        )
      );
    } else {
      setData((prev) => [
        ...prev,
        { id: prev.length + 1, name: newResignation.name, position: newResignation.title, status: newResignation.status },
      ]);
    }
    setShowCreatePopup(false);
    setShowPopup(false);
